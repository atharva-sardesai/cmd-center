...
const ACCESS_EVENTS = [
  ...
  { lat: 19.08, lng:  72.88, name: 'India Site — 41 expired access grants',      severity: 'elevated', overdue: 41, dept: 'HR'         },
...
export async function GET() {
  ...
  try {
    ...
    return NextResponse.json(
      { access_events, events: access_events, total: access_events.reduce((a, e) => a + e.overdue, 0), timestamp: new Date().toISOString() },
      { headers: { 'Cache-Control': 'public, s-maxage=60, stale-while-revalidate=120' } }
    );
  } catch {
    return NextResponse.json({ access_events: [], events: [] }, { status: 500 });
  }
}
